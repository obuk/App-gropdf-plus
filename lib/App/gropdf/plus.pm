...
#use App::gropdf::Type1;
#use App::gropdf::OpenType;

my $gotzlib = 0;
#type1# my $gotinline = 0;
my $gotexif = 0;
#base# my $xitcd     = 0;
#base# my $warnexit  = 0;
...
#base# my @obj;	# Array of PDF objects
#base# my $objct=0;    # Count of Objects
my $fct=0;      # Output count
my %fnt;	# Used fonts
my $lct=0;      # Input Line Count
my $src_name='';
#base# my %env;	# Current environment
#base# my %fontlst;    # Fonts Loaded
my $rot=0;      # Portrait
my %desc;       # Contents of DESC
my %download;   # Contents of downlopad file
#base# my $pages;      # Pointer to /Pages object
#base# my $devnm='devpdf';
...
#base# my $stream='';  # Current Text/Graphics stream
...
#base# my $textenccmap=''; # CMap for groff text.enc encoding
my @XOstream=();
my @PageAnnots={};
my $noslide=0;
#type1# my $ND='ND';
...
#base# my $stats=0;
#base# my $unicodemap;
#base# my $options = SUBSET + USESPACE + COMPRESS;
#base# my $PDFver=1.7;
#base# my @idirs;
...
#type1# my %sec;
#xxx# my $Glyphs='';
#type1# my (@glyphused, @subrused, %glyphseen);
#type1# my $newsub=4;
#type1# my $term="\n";
#type1# my @bl;
#type1# my %seac;
...
(undef,undef,$prog)=File::Spec->splitpath($0);
...
%info=('Creator' => "(groff version $cfg{GROFF_VERSION})",
	  'Producer' => "(gropdf version $cfg{GROFF_VERSION})",
	  'ModDate' => "($dt)",
	  'CreationDate' => "($dt)");

# [bug #55557] gropdf can execute arbitrary commands
# https://lists.gnu.org/r/bug-groff/2019-01/msg00029.html
...
foreach my $fontno (sort { $a <=> $b } keys %fontlst)
{
    my $fnt = $fontlst{$fontno}{FNT};
    if ($fnt->can('build_fontobject')) {
	$fnt->build_fontobject;
    } else {
	Die("could not call " . (ref $fnt) . "->build_fontobject");
    }
}
...
sub MakeMatrix
{
    ...
    # xxxxx - setup @mat for using OTF in italics and vertical writing.
    if ($thisfnt) {
	my ($a, $b, $c, $d);
	if (!$frot) {
	    if (!($thisfnt->{vertical})) {
...
sub LoadDownload
{
	...
	while (<$f>)
	{
	    ...
	    my ($foundry,$name,$file, $module)=split(/\t+/);
	    my $star = 0;
	    ...
	    # xxxxx - setting embed whether embedding is required into %download.
	    # this is because OTF needs to load even if not embed.
			...
			if (exists($pgnames{$want}))
			{
				...
				FIND: while (1)
				{
					...
					if ($ref eq $pages->{Kids}->[$j])
					{
						...
						Warn("expected 'switchtopage' parameter to be one of ",
...
sub PutObj
{
    ...
    # xxxxx - $thisono is the object number being processed by PutField.
    # It is used in the error message output when $fld is undef in
    # PutField.
...
# xxxxx - In addition to Type 1, OpenType/CFF is also supported.
sub LoadFont {
    my $fontno=shift;
    my $fontnm=shift;

    # Multiple foundries are available.
    ...
    $fontlst{$fontno}->{FNT}=$fnt_obj; # xxxxx

    Notice("Using nospace mode for font '$ofontnm'")
	if $fnt_obj->{nospace} and $options & USESPACE;
...
#type1# sub NextAlloc ...
#type1# sub decrypt_char ...
#type1# sub decrypt_exec_P ...
#type1# sub encrypt_exec ...
#type1# sub encrypt_char ...
#type1# sub map_subrs ...
#type1# sub subs_call ...
#type1# sub Subset ...
#type1# sub MarkSub ...
#type1# sub encrypt ...
#type1# sub encode_charstr ...
#type1# sub SubTag ...
...
This module separates Type1 font from the GNU groff 1.24.0 PDF driver
gropdf (written by Deri James) and adds OpenType/CFF support.

Please note that this module is intended for use with plenv.
